...
namespace UnityEditor.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		{	
			//Debuger.Log ("AddSearchPaths " + paths + key + (recursive?" recursive":"") + " " + (quoted?" quoted":""));
			bool modified = false;
			...
			foreach( string path in paths ) {
				...
				//Debuger.Log ("currentPath = " + currentPath);
		...
		public bool overwriteBuildSetting(string settingName, string settingValue) {
			Debuger.Log ("overwriteBuildSetting " + settingName + " " + settingValue);
			bool modified = false;
			
			if( !ContainsKey( BUILDSETTINGS_KEY ) ) {
				Debuger.Log ("creating key " + BUILDSETTINGS_KEY);
				this.Add( BUILDSETTINGS_KEY, new PBXSortedDictionary() );
			}
				
			if( !((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey( settingName ) ) {
				Debuger.Log("adding key " + settingName);
				...
				//Debuger.Log("key is string:" + settingName);
			...
			if( !((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[settingName]).Contains( settingValue ) ) {
				Debuger.Log("setting " + settingName + " to " + settingValue);
