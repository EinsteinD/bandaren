...
namespace UnityEditor.XCodeEditor 
{
	public class XCPlist
	{
		...
		const string BundleQueriesSchemes = "LSApplicationQueriesSchemes";
		const string PlistQueriesSchemes = "queriesschemes";
		...
		{
			Debuger.Log ("AddPlistItems: key=" + key);
			...
			{
				processQueriesSchemes((ArrayList)value, dict);
			} 
			else 
			{
		...
		private void processQueriesSchemes(ArrayList queriesschemes, Dictionary<string, object> dict)
		{
			List<object> bundleQueriesSchemes;
			if (dict.ContainsKey(BundleQueriesSchemes))
			{
				bundleQueriesSchemes = (List<object>)dict[BundleQueriesSchemes];
			}
			else
			{
				bundleQueriesSchemes = new List<object>();
			}

			foreach (object scheme in queriesschemes) {
				bundleQueriesSchemes.Add(scheme);
				plistModified = true;
			}
			dict[BundleQueriesSchemes] = bundleQueriesSchemes;
		}
