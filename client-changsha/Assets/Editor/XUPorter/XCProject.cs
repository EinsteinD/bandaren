	...
	public partial class XCProject : System.IDisposable
	{
		...
		public XCProject( string filePath ) : this()
		{
			if( !System.IO.Directory.Exists( filePath ) ) {
				Debuger.LogWarning( "XCode project path does not exist: " + filePath );
				return;
			}
			
			if( filePath.EndsWith( ".xcodeproj" ) ) {
				Debuger.Log( "Opening project " + filePath );
				this.projectRootPath = Path.GetDirectoryName( filePath );
				this.filePath = filePath;
			} else {
				Debuger.Log( "Looking for xcodeproj files in " + filePath );
				string[] projects = System.IO.Directory.GetDirectories( filePath, "*.xcodeproj" );
				if( projects.Length == 0 ) {
					Debuger.LogWarning( "Error: missing xcodeproj file" );
					return;
				}
				...
				//Debuger.Log ("projectRootPath adjusted to " + projectRootPath);
			...
			if( !_datastore.ContainsKey( "objects" ) ) {
				Debuger.Log( "Errore " + _datastore.Count );
				return;
			}
			...
			if( !string.IsNullOrEmpty( _rootObjectKey ) ) {
				...
				Debuger.LogWarning( "error: project has no root object" );
				_project = null;
				_rootGroup = null;
			}
		...
		public bool overwriteBuildSetting( string settingName, string newValue, string buildConfigName = "all") {
			Debuger.Log("overwriteBuildSetting " + settingName + " " + newValue + " " + buildConfigName);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				//Debuger.Log ("build config " + buildConfig);
				XCBuildConfiguration b = buildConfig.Value;
				if ( (string)b.data["name"] == buildConfigName || (string)buildConfigName == "all") {
					//Debuger.Log ("found " + b.data["name"] + " config");
					buildConfig.Value.overwriteBuildSetting(settingName, newValue);
					modified = true;
				} else {
					//Debuger.LogWarning ("skipping " + buildConfigName + " config " + (string)b.data["name"]);
				}
			}
			return modified;
		}
		...
		{
			Debuger.Log ("AddHeaderSearchPaths " + paths);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
		...
		{
			Debuger.Log ("AddLibrarySearchPaths " + paths);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
		...
		{
			//Debuger.Log("AddFile " + filePath + ", " + parent + ", " + tree + ", " + (createBuildFiles? "TRUE":"FALSE") + ", " + (weak? "TRUE":"FALSE") ); 
			
			PBXDictionary results = new PBXDictionary();
			if (filePath == null) {
				Debuger.LogError ("AddFile called with null filePath");
				return results;
			}
			...
			if( fileReference != null ) {
				Debuger.Log("File already exists: " + filePath); //not a warning, because this is normal for most builds!
				return null;
			}
				...
				switch( fileReference.buildPhase ) {
					...
					case "PBXResourcesBuildPhase":
						foreach( KeyValuePair<string, PBXResourcesBuildPhase> currentObject in resourcesBuildPhases ) {
							Debuger.Log( "Adding Resources Build File" );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
					case "PBXShellScriptBuildPhase":
						foreach( KeyValuePair<string, PBXShellScriptBuildPhase> currentObject in shellScriptBuildPhases ) {
							Debuger.Log( "Adding Script Build File" );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
					case "PBXSourcesBuildPhase":
						foreach( KeyValuePair<string, PBXSourcesBuildPhase> currentObject in sourcesBuildPhases ) {
							Debuger.Log( "Adding Source Build File" );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
					case "PBXCopyFilesBuildPhase":
						foreach( KeyValuePair<string, PBXCopyFilesBuildPhase> currentObject in copyBuildPhases ) {
							Debuger.Log( "Adding Copy Files Build Phase" );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
					case null:
						Debuger.LogWarning( "File Not Supported: " + filePath );
						break;
					default:
						Debuger.LogWarning( "File Not Supported." );
						return null;
				}
			}
			return results;
		...
		public PBXCopyFilesBuildPhase AddEmbedFrameworkBuildPhase()
		{
			...
			if (naviTarget == null)
			{
				Debuger.Log("Not found Correct NativeTarget.");
				return phase;
			}
		...
		{
			Debuger.Log( "Add Embed Framework: " + fileName );

			//Check if there is already a file
			PBXFileReference fileReference = GetFile( System.IO.Path.GetFileName( fileName ) );	
			if( fileReference == null ) {
				Debuger.Log("Embed Framework must added already: " + fileName);
				return;
			}
			...
			if (embedPhase == null)
			{
				Debuger.Log("AddEmbedFrameworkBuildPhase Failed.");
				return;
			}
		...
		{
			Debuger.Log("Folder PATH: "+folderPath);
			if( !Directory.Exists( folderPath ) ){
				Debuger.Log("Directory doesn't exist?");
				return false;
			}

			if (folderPath.EndsWith(".lproj")){
				Debuger.Log("Ended with .lproj");
			...
			foreach( string file in Directory.GetFiles( folderPath ) ) {
				...
				Debuger.Log("Adding Files for Folder");
		...
		{	
			...
			Debuger.Log( "Adding files..." );
			foreach( string filePath in mod.files ) {
				string absoluteFilePath = System.IO.Path.Combine( mod.path, filePath );
				this.AddFile( absoluteFilePath, modGroup );
			}

			Debuger.Log( "Adding embed binaries..." );
